...
import { 
  ...
  Clock, MoreVertical, Loader2, Phone, CheckSquare
} from 'lucide-react';
import { getAppointments, updateAppointmentStatus } from '../appointments/actions';
...
export default function TomorrowAppointmentsPage() {
  ...
  // Filters (Mặc định là ngày mai)
  const tomorrow = new Date();
  tomorrow.setDate(tomorrow.getDate() + 1);
  const tomorrowStr = tomorrow.toLocaleDateString('en-CA'); // YYYY-MM-DD
  
  const [filterDate, setFilterDate] = useState(tomorrowStr);
  ...
  return (
    <div className="space-y-6 pb-10 animate-fade-in">
      ...
      <div className="flex flex-col gap-1 mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Lịch hẹn ngày mai</h1>
        <p className="text-sm text-gray-500">Xem và quản lý các lịch hẹn đã đặt trước cho ngày mai</p>
      </div>
        ...
        <div className="overflow-x-auto min-h-[400px]">
          {appointments.length === 0 && !isLoading ? (
             <div className="p-20 text-center flex flex-col items-center justify-center">
               ...
               <h3 className="text-gray-900 font-bold mb-1">Không có lịch hẹn nào</h3>
               <p className="text-gray-500 text-sm">Chưa có bệnh nhân nào đặt lịch khám cho ngày mai.</p>
             </div>
          ) : (
